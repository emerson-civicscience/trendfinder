TFdateHandler <- function(data_start_dates, data_end_dates = NULL, time_period_length = NULL){
  ...
  if(is.null(data_end_dates)){
    ...
    if(is.null(time_period_length)){
    	time_period_length <- as.numeric(difftime(ymd(data_start_dates[length(data_start_dates)]), ymd(data_start_dates[length(data_start_dates) - 1])))
    }

		final_date <- ymd(data_end_dates[length(data_end_dates)]) + time_period_length

		data_end_dates <- c(data_end_dates, as.character(final_date))
	...
	if(final_date > today()){
		data_end_dates[length(data_end_dates)] <- as.character(today())
	}
