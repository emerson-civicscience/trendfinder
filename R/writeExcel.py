...
def writeExcel(pandas_df,
           data_colnames_wanted_py,
           chart_references_py,
           file_name_py): # Defines function
           
  pandas_df.loc[np.isnan(pandas_df['Banner Tag Order']), ['Banner Tag Order']] = 0
  pandas_df.loc[np.isnan(pandas_df['Stem Tag Order']), ['Stem Tag Order']] = 0
  ...
  if chart_references_py != None:
    for crosstab_loop in chart_references_py:
        ...
        crosstab_df_excel_cols_t = ['Stem Name', 'Banner Name'] + data_colnames_wanted_py
